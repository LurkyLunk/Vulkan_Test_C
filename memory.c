...
VkDeviceMemory InputBufferUniMemory1 = VK_NULL_HANDLE;
VkDeviceMemory InputBufferUniMemory2 = VK_NULL_HANDLE;
...
VkBuffer CreateBufferAndMemory(uint32_t size, VkDeviceMemory *deviceMemory,
                                        VkBufferUsageFlags usage, VkMemoryPropertyFlagBits memProperties)
{
    VkBuffer buffer;
                        ...
                        uint32_t inputSizeUni1, uint32_t inputSizeUni2)
{
    ...
    InputBufferUni1 = CreateBufferAndMemory(inputSizeUni1, &InputBufferUniMemory1,
                                        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | 
                                        VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    InputBufferUni2 = CreateBufferAndMemory(inputSizeUni2, &InputBufferUniMemory2,
                                        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | 
                                        VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void DestroyBuffers(void)
{
    ...
    vkFreeMemory(LogicalDevice, InputBufferUniMemory1, NULL);
    vkDestroyBuffer(LogicalDevice, InputBufferUni2, NULL);
    vkFreeMemory(LogicalDevice, InputBufferUniMemory2, NULL);
}
...
void CopyData(void *data, uint32_t size, int isWrite, VkDeviceMemory memory)
{
    ...
    if (isWrite) memcpy(address, data, size);
    else memcpy(data, address, size);
...
void CopyBufferToBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, uint32_t size)
{
    ...
    memset(&cmdBuffAllocInfo, 0, sizeof(cmdBuffAllocInfo));
    cmdBuffAllocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    cmdBuffAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    cmdBuffAllocInfo.commandPool = ComputeCmdPool;
...
void CopyToLocalBuffer(void *data, uint32_t size, VkBuffer buffer)
{
    ...
    VkBuffer tempBuffer = CreateBufferAndMemory(size, &tempMemory, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                                    VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
